function convertToMessageHTML(message) {
    return `<div class="message-group border-0 list-group-item list-group-item-action mb-0 p-0 text-${message.position} p${(message.position == 'left' ? 'l' : 'r')}-0" data-message-id="${message.id}" style="margin-top:1px;"> <!-- Ajax Element -->
            <div class="mb-0 w-100 bg-${(message.position == 'left' ? 'secondary' : 'info')} text-white mb-0" >
                <div class="d-flex w-100 justify-content-between mb-0 p-0 ${(message.position == 'left' ? '' : 'flex-row-reverse')}">
                    <img src="/messageboard/img/${message.avatar}" alt="Profile Image of ${message.name}" class="img-fluid rounded-square" style="width: 50px; max-height: 80px;">
                    <div class="w-100 mb-0">
                        ...
                        <div class="message-body text-truncate d-inline-block mb-0 px-0 w-100" style="word-break: break-word; text-overflow: ellipsis; max-width:100%; white-space:unset">
                            ${message.message}
                            ${message.message.length > 200 ? '<button class="badge badge-primary badge-sm border-0" id="see_more" type="button">See More</button>' : ''}
                        </div>
                    </div>
                </div>
    ...
    return `
        <a href="` + conversation_url + '/view/' + conversation.conversation_id + `"
            ...
            <div class="d-flex w-100  justify-content-start">
                <img src="/messageboard/img/${conversation.avatar}" alt="Profile Image of ${conversation.name}" class="img-fluid rounded-square" style="width: 50px;  margin-right: 10px;">
                <div>
